const board = [
    ...
    for (let j = 0; j < 9; j++) {
      ...
      if (board[i][j] === '0') {
        ...
        circle.addEventListener('click', function () {
          if (!gameEnded && !circle.classList.contains('red') && energyPlayer >= 20) {
  ...
  function updateTime() {
    ...
    timerInterval = setInterval(() => {
      ...
      if (seconds <= 180) {
        ...
        if (numRedCircles === 0 && energyPlayer < 20) {
          gameEnded = true;
          endGame(false);
        }
        checkWinCondition();
  ...
  boardElement.addEventListener('mouseup', function(e) {
      if (!gameEnded && e.target.classList.contains('circle') && energyPlayer >= 10) {
  ...
  function computerTurnGray() {
    ...
    const numberOfCirclesToTurnGray = Math.floor(redCircles.length / 1.75);
