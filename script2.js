const board = [
    ...
    for (let j = 0; j < 9; j++) {
      ...
      if (board[i][j] === '0') {
        ...
        circle.addEventListener('click', function () {
          if (!gameEnded && !circle.classList.contains('red') && energyPlayer >= 15) {
  ...
  function updateTime() {
    ...
    timerInterval = setInterval(() => {
      ...
      if (seconds <= 180) {
        ...
        if (numRedCircles === 0 && energyPlayer < 15) {
          gameEnded = true;
          endGame(false);
        }
        checkWinCondition();
  ...
  function computerTurnGray() {
    ...
    const numberOfCirclesToTurnGray = Math.floor(redCircles.length / 2);
